...
app.controller('OfferdonationCtrl', function ($scope, $http, $filter, UserService) {
    $scope.spinner = false;
...
app.controller('DonationCtrl', function ($scope, $http, $filter, UserService) {
    ...
    console.log("DonationCtrl User = " + JSON.stringify(UserService.getLoggedIn()));
    $scope.login_email = UserService.getLoggedIn().email;
    $scope.found = '';
    $scope.result = '';
    $scope.groupusers = [];
    ...
    $scope.SendOffer = function (offer) {
        ...
        $http({
            ...
            notifyUsersInGroup("FROM-" + offer.city.trim().toUpperCase() + "-" +
                offer.from.trim().toUpperCase(), offer.from, filteredtime, offer.name, offer.phone);
    ...
    var notifyUsersInGroup = function (group, from, time, by, phone) {
        ...
        $http({
            ...
            $scope.SendPush(gcmids, "A new donation created by " + by + "(ph: " + phone + "), pickup at " + time + " from " + from);
 ...
 //       console.log("checkIfPushAllowedNow: received input - " + JSON.stringify(settingsObject));
        if (settingsObject === undefined || !settingsObject)
            return true;
 ...
 //       alert("Sending Push TO User With UUID=" + uuid);
    ...
    $scope.AcceptDonation = function (row, status) {
        ...
        var updateURL = "http://localhost:9000/acceptdonation?uuid=" + row.uuid
            ...
            + "&receiver_uuid=" + loggedinUser.uuid
            + "&received_time=" + filteredtime
            + "&status=" + status;
        ...
        $http({
            ...
            if (response.data === "Already Accepted") {
                ...
                $scope.alldonations = true;
                $scope.cancel = true;
                SendPushToUserByEmail(row.email, "donation accepted by " + loggedinUser.fullname);
            }
    ...
    $scope.GetdonationAcceptances = function (row, cancel) {
        ...
        $http({
            ...
            if (cancel)
                $scope.Canceldonation(row, false);
        }, function errorCallback(error) {
            // called asynchronously if an error occurs
            // or server returns response with an error status.
            $scope.spinner = false;
    ...
    $scope.CancelOffer = function (row) {
        ...
        $http({
            ...
            SendPushToUser(row.receiver.receiver_uuid, "A donation offered by " + $scope.fullname + " has been cancelled");
        }, function errorCallback(error) {
            // called asynchronously if an error occurs
            // or server returns response with an error status.
            $scope.spinner = false;
